...
class Money:
    ...
    def sell(self, product):
        ...
        if money_sum > product_sum:
            ...
            if self.discount < 0.2:
                if int((product_sum * self.curs["UAH"].get(self.currency)) / 50000) >= 1:       # Так як у нас знижка нараховується після покупки товара на сумму
                    discount = int((product_sum * self.curs["UAH"].get(self.currency)) / 50000) * 0.05     # евівалентний гривні, то ми до цього єквівалента і приводимо, не
                    self.discount += discount                                                   # зважаючи на те, яка у нас валюта гамaнця і товару
            else:
                self.discount = 0.2
            self.cili = int(money_sum/100)
            self.pennies = int(str(int(money_sum))[-2::])                   # переводимо в інт, щоб прибрати дробові чісла після переведення вілюти
        ...
        return f'З вас списано: {product}\nЗалишок: {self.cili}.{viz_pennies} {self.currency} discount = {self.discount * 100}%'
...
class Product(Money):
    def __init__(self, name, cili, pennies, currency):
        super(Product, self).__init__(cili, pennies, currency)
        self.path = os.path.join("", "Menu.txt")
        self.name = name
        file = open(self.path, "a")
        file.write(f"{self.name}, {self.cili}, {self.pennies}, {self.currency}\n")
        file.close()

    def __repr__(self):
        return f"{self.name},{self.cili}, {self.pennies}, {self.currency}"

    def allTovar(self):
        file = open(self.path, "r")
        menu = file.read().split("\n")
        return '\n'.join(menu)

money = Money(2000, 10, "UAH")
print(money)
# coffe_shop = Product()
coffe = Product("coffe", 12, 00, "DOL")
rogalik = Product("rogalik", 520, 00, "UAH")
# print(Product("coffe", 42, 00, "UAH").allTovar())

print(money.sell(coffe))
print(money.sell(rogalik))
# coffe = Money(42, 00, "UAH")
# cofee = Product()
# coffe_shop.add_menu("coffe", 42, 10, "UAH")
# coffe_shop.add_menu("rogalik", 32, 20, "UAH")